...
main(int argc, char* argv[])
{
    ...
    // Check if the packet size exceeds an assumed MTU of 1500 bytes if not log error
    uint32_t packetSize = 1600;
    if (packetSize > 1500) {
        NS_LOG_ERROR("Packet size exceeds MTU size of 1500 bytes! Potential fragmentation or error."); // LOG ERROR
    }
